...
\subsubsection{Simplified System}
...
Several different cost functions were tested to arrive at a cost function for parameter tuning. It was assumed that the \textit{assistie} system was nonobservable and therefore could not be used for the optimization of the parameters. The only criteria used for the cost functions were those that could be measured by the \textit{assistor} system. Three different cost functions were tested, one based on position, another on velocity, and finally, one used both position and velocity.
