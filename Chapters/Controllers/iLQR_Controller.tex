...
One of the primary problems with iLQR is that it is diffcult for real time control for real-time control. A model predictive controller (MPC) could be use to solve some of the limitation of LQR \cite{bemporad2002explicit}. One of the main difference between LQR and MCP is that LQR are infinite horizon controller and MPC are finite horizon controllers, which have been used for real time control \cite{wang2009model}. However this become more difficult when attempting to use a iLQR instead of the LQR. This is due to computational time the line search step, which cannot be done in real time because it is unknown how many iteration this step will take.  This limitation can somewhat over be overcome by using parallel processing in a separate thread, which would run parallel to the main controller thread \cite{MPC}. This solution however is equivalent to just use more computing power and may is not a solution that is possible on all systems. If a better initial guess could be made, then it is possible to reduce the line search loop however this would not work for all possible trajectories. 
